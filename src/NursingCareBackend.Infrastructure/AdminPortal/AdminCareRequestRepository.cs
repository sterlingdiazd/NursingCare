...
public sealed class AdminCareRequestRepository : IAdminCareRequestRepository
{
      ...
      CancellationToken cancellationToken)
    {
        ...
        return new AdminCareRequestDetail(
          ...
          BankReference: careRequest.BankReference,
          PaidAtUtc: careRequest.PaidAtUtc,
          VoidReason: careRequest.VoidReason,
          VoidedAtUtc: careRequest.VoidedAtUtc,
          ReceiptNumber: careRequest.ReceiptNumber,
          ReceiptGeneratedAtUtc: careRequest.ReceiptGeneratedAtUtc);
    }
        ...
        var timeline = new List<AdminCareRequestTimelineEvent>
    {
      new(
        ...
        if (careRequest.InvoicedAtUtc.HasValue)
        {
            timeline.Add(new AdminCareRequestTimelineEvent(
              Id: $"invoiced:{careRequest.Id}",
              Title: "Facturada",
              Description: $"Factura: {careRequest.InvoiceNumber}",
              OccurredAtUtc: careRequest.InvoicedAtUtc.Value));
        }

        if (careRequest.PaidAtUtc.HasValue)
        {
            timeline.Add(new AdminCareRequestTimelineEvent(
              Id: $"paid:{careRequest.Id}",
              Title: "Pagada",
              Description: $"Ref: {careRequest.BankReference}",
              OccurredAtUtc: careRequest.PaidAtUtc.Value));
        }

        if (careRequest.VoidedAtUtc.HasValue)
        {
            timeline.Add(new AdminCareRequestTimelineEvent(
              Id: $"voided:{careRequest.Id}",
              Title: "Anulada",
              Description: careRequest.VoidReason ?? string.Empty,
              OccurredAtUtc: careRequest.VoidedAtUtc.Value));
        }

        if (careRequest.ReceiptGeneratedAtUtc.HasValue)
        {
            timeline.Add(new AdminCareRequestTimelineEvent(
              Id: $"receipt:{careRequest.Id}",
              Title: "Recibo generado",
              Description: $"No. {careRequest.ReceiptNumber}",
              OccurredAtUtc: careRequest.ReceiptGeneratedAtUtc.Value));
        }
    ...
    {
        if (careRequest.Status is CareRequestStatus.Completed
            or CareRequestStatus.Invoiced
            or CareRequestStatus.Paid
            or CareRequestStatus.Voided)
        {
            return false;
        }
