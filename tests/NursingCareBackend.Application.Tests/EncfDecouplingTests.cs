...
public sealed class EncfDecouplingTests : IDisposable
{
    ...
    private sealed class DbPaymentValidationRepository : IPaymentValidationRepository
    {
        ...
        public Task<bool> IsBankReferenceUsedAsync(string bankReference, Guid excludeCareRequestId, CancellationToken cancellationToken)
            => Task.FromResult(false);
    }

    private sealed class NoopReceiptHandler : IGenerateReceiptHandler
    {
        public Task<GenerateReceiptResponse> Handle(GenerateReceiptCommand command, CancellationToken cancellationToken)
            => Task.FromResult(new GenerateReceiptResponse(Guid.NewGuid(), "REC-TEST-0001", string.Empty));
    }
